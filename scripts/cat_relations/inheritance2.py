...
class InheritanceDb:
    ...
    def _load_inheritance(self, cat, save=False):
        ...
        for parent_id in (cat.parent1, cat.parent2, cat.parent3):
            if parent_id:
                self._cat_to_rels[cat.ID].parents.append(
                    {"relation_type": RelationType.SURROGATE if parent_id in cat.surrogate_parents else RelationType.BLOOD, "cat_id": parent_id}
                )
                self._cat_to_rels[parent_id].children.append(
                    {"relation_type": RelationType.BLOOD, "cat_id": cat.ID}
                )
        if cat.parent1 or cat.parent2 or cat.parent3:
            self._cat_to_litter[cat.ID] = (
                frozenset((cat.parent1, cat.parent2, cat.parent3)),
