...
from scripts.rabbit.enums import CatAge, CatRank, CatSocial, CatGroup, CatStanding
...
def get_current_season():
    ...
    game.warren.current_season = constants.SEASON_CALENDAR[index]
...
def create_new_cat_block(
    ...
    for _tag in attribute_list:
        ...
        if match.group(1) in Rabbit.age_moons:
            min_age, max_age = Rabbit.age_moons[CatAge(match.group(1))]
    ...
    if rank and not age:
        if rank in [
            ...
            age = randint(
                Rabbit.age_moons[CatAge.ADOLESCENT][0],
                Rabbit.age_moons[CatAge.ADOLESCENT][1],
            )
    ...
    if "kittypet" in attribute_list:
        ...
        if other_clan:
            ...
            cat_group = choice(game.warren.other_clans)
    else:
    ...
    chosen_cat: Optional["Rabbit"] = None
...
def create_new_cat(
    ...
    if (
        ...
        original_group = choice(game.warren.other_clans)
    ...
    for key_age in Rabbit.age_moons.keys():
        if moons in range(Rabbit.age_moons[key_age][0], Rabbit.age_moons[key_age][1] + 1):
            age: CatAge = key_age
            break
...
def history_text_adjust(text, other_clan_name, warren, other_cat_rc=None):
    ...
    if "c_n" in text:
        text = text.replace("c_n", warren.displayname)
...
def ongoing_event_text_adjust(Rabbit, text, warren=None, other_clan_name=None):
    ...
    if "lead_name" in text:
        kitty = Rabbit.fetch_cat(game.warren.leader)
        cat_dict["lead_name"] = (str(kitty.name), choice(kitty.pronouns))
    if "dep_name" in text:
        kitty = Rabbit.fetch_cat(game.warren.deputy)
        cat_dict["dep_name"] = (str(kitty.name), choice(kitty.pronouns))
    if "med_name" in text:
        kitty = choice(
            find_alive_cats_with_rank(Rabbit, [CatRank.MEDICINE_CAT], working=True)
        )
    ...
    if warren:
        ...
        if game.warren is None:
            ...
            clan_name = str(game.warren.displayname)
...
def event_text_adjust(
    ...
    *,
    ...
    # dep_name
    if "dep_name" in text:
        captain = Rabbit.fetch_cat(game.warren.deputy)
...
def leader_ceremony_text_adjust(
    ...
    text = text.replace("c_n", str(game.warren.displayname) + "Clan")
...
def ceremony_text_adjust(
    ...
    clanname = str(game.warren.displayname + "Warren")
    ...
    cat_dict = {
        ...
        "l_n": (
            (str(game.warren.leader.name), choice(game.warren.leader.pronouns))
