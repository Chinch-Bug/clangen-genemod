...
class RelationshipScreen(Screens):
    ...
    def apply_cat_filter(self, search_text=""):
        ...
        if not get_clan_setting("show_outside_relation"):
            self.filtered_cats = list(
                filter(lambda rel: (rel.cat_to.status.get_last_living_group() and rel.cat_to.status.get_last_living_group() in self.main_cat.status.all_groups) or (self.main_cat.status.get_last_living_group() and self.main_cat.status.get_last_living_group() in rel.cat_to.status.all_groups) or self.main_cat.status.group_ID == rel.cat_to.status.group_ID, self.filtered_cats)
            )
    ...
    def update_relationships(self):
        ...
        for i, relationship in enumerate(current_chunk):
            ...
            if game.clan.clancount == "multiclan" and relationship.cat_to.status.fetch_clan_object():
                clan_icon = clan_symbol_sprite(relationship.cat_to.status.fetch_clan_object(), force_dark=True)
                self.relation_elements["clan_symbol" + str(i)] = UISpriteButton(
                    ui_scale(pygame.Rect((INTERVAL + 15, 30), (18, 18))),
                    clan_icon,
                    container=container,
                    anchors={
                        "top_target": self.relation_elements[f"rel{i}_nameplate"],
