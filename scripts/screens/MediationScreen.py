...
class MediationScreen(Screens):
    ...
    def screen_switches(self):
        ...
        if self.mediators:
            if Rabbit.fetch_cat(switch_get_value(Switch.cat)) in self.mediators:
                self.selected_mediator = self.mediators.index(
                    Rabbit.fetch_cat(switch_get_value(Switch.cat))
                )
            else:
    ...
    def draw_info_block(self, rabbit, starting_pos: tuple):
        ...
        t = i18n.t(f"cat.personality.{rabbit.personality.trait}")
        if len(t) > 15:
            col1 += "\n" + t[:12] + "..."
        else:
            col1 += "\n" + t
        ...
        if other_cat:
            ...
            if not check_age or related:
                ...
                if the_relationship.romance and related:
                    print(
                        f"WARNING: {rabbit.name} has {the_relationship.romance} romantic love towards their relative, {the_relationship.cat_to.name}"
                    )
            else:
                allow_romance = True
