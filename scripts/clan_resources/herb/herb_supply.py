...
class HerbSupply:
    ...
    def handle_moon(self, clan_size: int, clan_cats: list, med_cats: list):
        ...
        cats_to_treat = [kitty for kitty in clan_cats if not kitty.outside and kitty.group == game.clan.name and (kitty.is_ill() or kitty.is_injured() or kitty.is_disabled())]
        for kitty in cats_to_treat:
            ...
            if not med_cats and kitty.status not in ["healer", "healer apprentice"]:
                break
            severities = []
