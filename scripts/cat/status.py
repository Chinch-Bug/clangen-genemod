...
class Status:
    ...
    def send_to_afterlife(self, target_ID: str = None):
        ...
        if game.clan:
            cat_clan = self.fetch_clan_object()
            self.add_to_group(new_group_ID=cat_clan.instructor.status.group_ID if cat_clan.instructor else game.clan.instructor.status.group_ID)
        else:
    ...
    def is_near(self, group_ID: str = None) -> bool:
        ...
        """
        for entry in self.standing_history[::-1]:
            if (not group_ID or entry.get("group") == group_ID) and entry.get("near"):
                return True
