...
def get_other_cat_for_thought(
    ...
    if main_cat.status.group.is_afterlife():
        ...
        while cat_list and (
            ...
            or other_cat.status.get_last_living_group() != main_cat.status.get_last_living_group()
        ):
...
def _load_group(main_cat: "Cat", other_cat: "Cat", biome, season, camp, ageup=False):
    """
...
def new_thought(main_cat: "Cat", other_cat: "Cat", biome, season, camp, ageup=False):
    """
    ...
    try:
        ...
        if (str(main_cat.name)).replace(" ", "").replace(
            ...
            chosen_thought_group = choice(
                _load_group(main_cat, other_cat, biome, season, camp, ageup)
            )
    ...
    if "perm_conditions" in thought:
        if "m_c" in thought["perm_conditions"]:
            ...
            valid_conditions = [
                ...
                if key in thought["perm_conditions"]["m_c"] and value in revealed_conditions
            ]
        ...
        if "r_c" in thought["perm_conditions"] and random_cat:
            ...
            valid_conditions = [
                ...
                if key in thought["perm_conditions"]["r_c"] and value in revealed_conditions
            ]
