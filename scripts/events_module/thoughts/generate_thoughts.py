...
def _filter_list(inter_list: list, main_cat: "Cat", other_cat: "Cat", other_clan_id=str, ageup=False) -> list:
    """
...
def _load_group(thought_type: CatThought, main_cat: "Cat", other_cat: "Cat", other_clan_id=str, ageup=False):
    """
    ...
    # LIVING CATS
        ...
        if main_cat.status.is_lost():
            ...
            if prior_rank:
                ...
                thoughts.extend(_load_file(f"{start_path}/while_lost/{prior_rank}.json"))
        else:
            thoughts.extend(_get_general(main_cat, new_path))
            thoughts.extend(_get_exiled_and_former(main_cat, new_path))
    ...
    final_thoughts = _filter_list(thoughts, main_cat, other_cat, other_clan_id, ageup=ageup)
    main_cat.age = og_age
...
def new_thought(
    thought_type: CatThought, main_cat: "Cat", other_cat: "Cat", other_clan_id: str, ageup=False
):
    """
    ...
    try:
        ...
        if (str(main_cat.name)).replace(" ", "").replace(
            " ", ""
        ).lower() == "rickastley":
            return i18n.t("defaults.rickroll")
        else:
            chosen_thought_group = choice(
                _load_group(thought_type, main_cat, other_cat, other_clan_id, ageup)
            )
...
def _constraints_fulfilled(
    ...
    if thought.location:
        if not event_for_location(thought.location, main_cat.status.fetch_clan_object(game.clan)):
            return False
