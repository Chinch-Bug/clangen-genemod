...
def _handle_moving_on(cat: Cat, disable_random: bool = False):
    ...
    for mate_id in cat.mate:
        ...
        dead_or_gone = (
            not mate.status.group_ID == cat.status.group_ID and mate.status.moons_as >= 4
        )
        ...
        if "grief stricken" not in cat.illnesses and dead_or_gone:
            ...
            if random.random() <= chance or disable_random:
                ...
                game.cur_events_list.append(
                    EventInformation(
                        text, ["relation"], cat_dict={"m_c": cat, "r_c": mate}, clan=cat.status.group_ID
                    )
                )
                cat.unset_mate(mate)
...
def _get_existing_mates(cat_from: Cat, cat_to: Cat) -> tuple[list[str], list[str]]:
    ...
    existing_from_cat_mates = [
        ...
        if cat_from.fetch_cat(mate).status.group_ID == cat_from.status.group_ID
    ]
    existing_to_cat_mates = [
        mate
        for mate in cat_to.mate
        if cat_to.fetch_cat(mate).status.group_ID == cat_to.status.group_ID
    ]
