...
class GroupEvents:
    ...
    @staticmethod
    def start_interaction(rabbit: Rabbit, interact_cats: list) -> list:
        ...
        abbreviations_cat_id = {
            "m_c": rabbit.ID
        }  # keeps track of which abbreviation is which rabbit
    ...
    def get_main_cat_interactions(
        ...
        abbreviations_cat_id : dict
            dictionary which contains all abbreviation with the belonging rabbit id
        ...
        main_cat = Rabbit.all_cats[abbreviations_cat_id["m_c"]]
        ...
        for interact in interactions:
            ...
            for abbreviation in dictionary:
                ...
                if abbreviation in interact.status_constraint:
                    ...
                    status_ids = [
                        rabbit.ID
                        for rabbit in interact_cats
                        if rabbit.status in interact.status_constraint[abbreviation]
                    ]
                else:
                    # if there is no constraint, add all ids to the list
                    status_ids = [rabbit.ID for rabbit in interact_cats]
                ...
                if abbreviation in interact.skill_constraint:
                    skill_ids = [
                        rabbit.ID
                        for rabbit in interact_cats
                        if rabbit.skill in interact.skill_constraint[abbreviation]
                    ]
                else:
                    skill_ids = [rabbit.ID for rabbit in interact_cats]

                if abbreviation in interact.trait_constraint:
                    trait_ids = [
                        rabbit.ID
                        for rabbit in interact_cats
                        if rabbit.personality.trait
                        in interact.trait_constraint[abbreviation]
                    ]
                else:
                    trait_ids = [rabbit.ID for rabbit in interact_cats]

                # only add the id if it is in all other lists
                for cat_id in (rabbit.ID for rabbit in interact_cats):
                    if (
    ...
    def set_abbreviations_cats(
        ...
        free_to_choose = [rabbit.ID for rabbit in interact_cats]
    ...
    def relationship_allow_interaction(
        ...
        for name, rel_constraint in interaction.relationship_constraint.items():
            ...
            cat_from = Rabbit.all_cats[cat_from_id]
            cat_to = Rabbit.all_cats[cat_to_id]
    ...
    def cat_allow_interaction(
        ...
        for abbr, constraint in interaction.status_constraint.items():
            ...
            relevant_cat = Rabbit.all_cats[abbreviations_cat_id[abbr]]
    ...
    def influence_general_relationship(
        ...
        for rabbit in abbreviations_cat_id:
            abbreviations_cat.append(Rabbit.fetch_cat(rabbit))
    ...
    def injuring_cats(chosen_interaction, abbreviations_cat_id):
        ...
        for abbreviations, injury_dict in chosen_interaction.get_injuries.items():
            if "injury_names" not in injury_dict:
                ...
                continue
            injured_cat = Rabbit.all_cats[abbreviations_cat_id[abbreviations]]
    ...
    def prepare_text(text: str, abbreviations_cat_id: dict) -> str:
        ...
        for abbr, cat_id in abbreviations_cat_id.items():
            replace_dict[abbr] = (
                str(Rabbit.all_cats[cat_id].name),
                choice(Rabbit.all_cats[cat_id].pronouns),
            )
