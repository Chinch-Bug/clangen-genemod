...
class Pregnancy_Events:
    ...
    def handle_having_kits(cat, warren):
        ...
        # check if there is a cat in the warren for the second parent
    ...
    @staticmethod
    def handle_adoption(cat: Rabbit, other_cat=None, warren=game.warren):
    ...
    @staticmethod
    def handle_zero_moon_pregnant(cat: Rabbit, other_cat=None, warren=game.warren):
        ...
        if get_clan_setting("same sex birth"):
            ...
            if not other_cat and random.randint(0, 1):
                amount = Pregnancy_Events.get_amount_of_kits(cat)
                kits = Pregnancy_Events.get_kits(amount, cat, None, warren)
                print_event = i18n.t(
            ...
            text = event_text_adjust(Rabbit, text, main_cat=cat, clan=warren)
            game.cur_events_list.append(
            ...
            text += choice(Pregnancy_Events.PREGNANT_STRINGS[f"{severity[0]}_severity"])
            text = event_text_adjust(Rabbit, text, main_cat=pregnant_cat, clan=warren)
            game.cur_events_list.append(
                Single_Event(
    ...
    @staticmethod
    def handle_one_moon_pregnant(cat: Rabbit, warren=game.warren):
        """Handles if the cat is one moon pregnant."""
        if cat.ID not in warren.pregnancy_data.keys():
            return

        # if the pregnant cat killed meanwhile, delete it from the dictionary
        if cat.dead:
            del warren.pregnancy_data[cat.ID]
            return
        ...
        warren.pregnancy_data[cat.ID]["amount"] = amount
        ...
        text = event_text_adjust(Rabbit, text, main_cat=cat, clan=game.warren)
        game.cur_events_list.append(
            Single_Event(text, "birth_death", cat_dict={"m_c": cat})
        )

    @staticmethod
    def handle_two_moon_pregnant(cat: Rabbit, warren=game.warren):
        ...
        kits_amount = warren.pregnancy_data[cat.ID]["amount"]
        if (
            kits_amount == 0
        ):  # safety check, sometimes pregnancies were ending up with 0 due to save rollbacks
            kits_amount = 1
        other_cat_id = warren.pregnancy_data[cat.ID]["second_parent"]
        other_cat = Rabbit.all_cats.get(other_cat_id)

        kits = Pregnancy_Events.get_kits(kits_amount, cat, other_cat, warren)
        ...
        print_event = event_text_adjust(
            Rabbit, print_event, main_cat=cat, random_cat=other_cat, clan=game.warren
        )
    ...
    @staticmethod
    def get_second_parent(cat, warren):
        """
    ...
    @staticmethod
    def get_kits(
        kits_amount, cat=None, other_cat=None, warren=game.warren, adoptive_parents=None
    ):
        ...
        # check other cats of Warren for siblings
