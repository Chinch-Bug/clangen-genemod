...
from scripts.rabbit.enums import CatRank, CatAge
...
def event_for_tags(tags: list, rabbit, other_cat=None) -> bool:
    ...
    if hasattr(rabbit, "ID"):
        ...
        if (
            ...
            and not other_cat.is_potential_mate(rabbit)
        ):
            return False
    ...
    for _tag in tags:
        ...
        for rank in ranks:
            ...
            if rank in [
                CatRank.LEADER,
                CatRank.DEPUTY,
            ] and not find_alive_cats_with_rank(rabbit, [rank]):
                return False

            if (
                rank not in [CatRank.LEADER, CatRank.DEPUTY]
                and not len(find_alive_cats_with_rank(rabbit, [rank])) >= 2
            ):
                return False
...
def _check_cat_not_skills(cat, skills: list) -> bool:
    ...
    for _skill in skills:
        skill_info = _skill.split(",")
        if len(skill_info) < 2:
            print("Rabbit not_skill incorrectly formatted", _skill)
            continue
