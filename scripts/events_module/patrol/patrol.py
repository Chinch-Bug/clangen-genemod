...
class Patrol:
    ...
    def __init__(self):
        ...
        self.outcome_cats: TypedDict(
            "outcome_cats", {PatrolOutcome.SUCCESS: dict[str, Cat], PatrolOutcome.FAILURE: dict[str, Cat]}
        ) = {PatrolOutcome.SUCCESS: {}, PatrolOutcome.FAILURE: {}}
        ...
        for block in romantic_event.relationship_constraint:
            if "can_romance" in block["constraints"]:
                ...
                cats_to = gather_cat_objects(
                    Cat, block["cats_to"], event=self, involved_cats=involved_cats, clan=self.clan
                )
    ...
    def calculate_success(
        ...
        for abbr, constraints in success_outcome.involved_cats.items():
            ...
            if stat_block := constraints.get("stat"):
                cat = self.outcome_cats[PatrolOutcome.SUCCESS].get(abbr)
                if not cat:
