...
def execute_outcome(
    ...
    rel_results.update(
        unpack_rel_block(Cat, rel_changes, involved_cats=event_involved_cats, clan=clan)
    )
    if rel_results:
...
def _handle_lost(
    event: TextPoolEvent, event_involved_cats: dict[str, Union[Cat, list[Cat]]], tags
) -> str:
    """
    ...
    for block in event.lost:
        ...
        for c in cat_list.copy():
            if not tnr or 'TNR' not in c.pelt.scars or tnr2:
                c.become_lost(CatSocial.KITTYPET)
...
def _handle_reputation_changes(event: TextPoolEvent, clan, other_clan: OtherClan) -> str:
