...
- ClanGen is a game that relies on random event generation. This means that whatever you're writing needs to function for all rabbits it might generate for, under all circumstances it can be generated in.

!!! caution
    No assumptions should be made about the gender of any rabbit. Nothing in the game is completely gender-locked, including pregnancy under certain settings, so all text should take this into account.
...
- The The Lightless is not fully implemented yet, and flavor text should not mention it too much at the moment. It exists, but there hasn't been a consensus on the exact implementation yet for things like training in the The Lightless, or how exactly it operates in ClanGen. For example, if it's a secret to all except those who are chosen to train there, or if it's well-known to all Warren rabbits.
...
!!! caution
    ClanGen contains content that is designed to cause distress. There is no way around it, and it is intentional, and it is **not malicious**. Players are supposed to care about the death of a rabbit, or a rabbit being injured, or the Warren not having enough food. Writers for ClanGen spend our time trying to provoke emotions in players, both good, positive, happy emotions, and sad ones, or feelings of injustice, or loss. Video games as a medium, especially a story generator like ClanGen, are designed to need active investment from a player on many levels, including emotional.

- There is an important difference between upsetting content and graphic content. 
- ClanGen follows the rabbits canonical levels of graphic content and gore. **This is not a non-graphic book series.** For example, in rabbits canon Scourge gets an extensive scene where he takes all of Tigerstar(the first)'s lives at once by ripping out his throat. This would be the absolute maximum graphic content allowed in ClanGen.
...
- Graphical content is not limited to violent content. Illness, injury, pregnancy - no one wants to read a play by play of a rabbit giving birth in extreme graphical detail. 
...
- People have drastically different levels of comfort for graphical content for kits (and sometimes rusasirahs) than for adult characters. This does not mean that nothing upsetting should ever be allowed to happen to kits, but it's worth considering. 
...
All biomes are different from each other and this should be taken into account when writing (e.g a generalized hunting patrol shouldn't mention fishing in the desert). If the content you're writing is generalized (available in any biome), then you have to be careful to make it _truly_ general in nature, able to be triggered no matter _what_ the circumstances. Therefore, under most circumstances, making biome (or even burrow) specific content is preferred for a more detailed, rich gameplay experience. Each biome also has different prey available - see the randomized prey lists for inspiration, but be creative (though realistic) with your own prey choices. Rabbits cannot take down giraffe, but there's plenty of unique things in the right size range for them to hunt you _could_ include in your content. 
...
- **Difficulty of interactions** with rabbits outside the warren refers to both how frequent and how dangerous interactions with rogues and loners are. 
- **Amount of Twolegs interactions** refers to how often Twolegs are seen in that biome and affect the Warren rabbits. 
...
- Beach has a **medium** difficulty of interactions with rabbits outside the warren.
...
- Desert has an **easy** difficulty of interactions with rabbits outside the warren.
- Desert has a **low** amount of Twolegs interactions.

!!! tip
    Take into account how the highly seasonal desert will affect the difficulty of the patrol. Greenleaf is dangerous in the desert - more sunlight is worse for the Warren. This results in the most interesting feature of the desert compared to other biomes - **desert has flipped seasons**. Leaf-bare, the rainy season, is a time of plenty, while greenleaf, the dry season, is a time of scarcity. Fresh water is an important resource here in ways not experienced in other biomes.

    Rabbits outside the clans and Twolegs are both rare in the desert, as it's not the most hospitable environment. While snow and sleet do fall in the desert on rare occasions, mostly leaf-bare has rain, not snow. 
...
- Forest has a **medium** difficulty of interactions with rabbits outside the warren.
- Forest has a **medium** amount of Twolegs interactions.

!!! tip
    Forest is the 'default' biome, intended to most closely mimic the experience and risks of the rabbit rabbits canon environment. This means that content created for the forest biome should more closely mimic the content in the rabbits rabbit books than other biomes. You should make increased numbers of skill checks for the climber skill in this biome.
...
- Mountainous has an **easy** difficulty of interactions with rabbits outside the warren.
- Mountainous has a **low** amount of Twolegs interactions.

!!! tip
    Living in the mountains isn't easy, but it affords the Clans who can survive there safety and isolation that are more difficult to come by then in other biomes. Snow falls in leaf-fall and newleaf, not just in leaf-bare, and leaf-bare has a deeper and colder chill than is display in rabbit rabbits canon. 
...
- Plains have a **medium** difficulty of interactions with rabbits outside the warren.
...
- Wetlands have a **medium** difficulty of interactions with rabbits outside the warren.
...
- **Warren** and **Clanmate** is always capitalized, even when Warren is a suffix
- **Gathering** is always capitalized
- **Inle** is always capitalized
- **The Lightless** is always capitalized
...
- If you want the rabbits to swear, **fox-dung** and **mouse-dung** are always hyphenated
...
- Rabbit rabbits canonically doesn't take place in a real setting, having both cougars and English woodland creatures
...
- Some biomes may require further research in order to give them a diverse set of animals to encounter, such as wetlands and desert which are very different to the original rabbits setting. So go wild finding cool animals from those biomes who you want to see in the game!
...
- Megafauna, in the ClanGen context, is any creature that is so far out of a rabbit's size range as to make a contest between a rabbit and that creature fundamentally pointless.
...
- What's important for ClanGen is that we want to make the game feel like these big creatures exist in the same world as the Warren, but that the Warren is not _constantly interacting or in danger from them_. So, for example, a plain patrol about scenting lion scat? Great. A plain patrol about being chased by a lion? That one better be _rare_ (and very dangerous). 
- You're encouraged to think of interesting interactions with megafauna that increase their presence in the game without subjecting the player Warren to undue amounts of danger! For example, a mountain Warren scavenging and eating the discarded placenta of a moose in newleaf is a wonderful source of prey, that implies moose are present in the world without requiring the Warren to risk interacting with it. Or what myths might a plains Warren tell about elephant? The beach Clans swap herbs and prey with dolphins without risking getting eaten by them.
- Basically, we want to feel like there are these massive creatures in the Warren's world, without having to solely focus gameplay on why the Warren isn't immediately getting eaten by a tiger.

#### How Much Biological Accuracy to Include:
- Please try and keep animals behaving in a way that is realistic to how they behave in real life. For example, while there are many fox patrols, unfortunately it isn't very realistic to have them befriend the rabbits (nor is it canonical, with Hollyleaf's fox kit trying to attack her even though she saved its life)
- If you want to include fun biological facts, this is encouraged! For example, the various fox patrols all have different success likelihoods and have the text explain that particular fox species' size compared to a rabbit, so swift foxes are less dangerous than red ones. 
- For patrols: 
    - Keep in mind that it's absolutely okay and encouraged to make certain animal encounters restricted to certain seasons, but that if it's possible to encounter an animal, it should be possible to encounter it with any size of patrol - i.e if introducing a dangerous predator, make sure it can still be encountered by single rabbit patrols, even if those patrols are very lethal and difficult.
    - Keep in mind that the size of a patrol should have a major impact on how difficult things are to accomplish. For example, while any size of patrol can encounter the maned wolf in the plains biome, only a full 6 rabbit patrol can try to fight the maned wolf
...
- The classics:
    ...
    - rabbits vs rabbit’s vs rabbits’
        - Plurals and possessives! Basically, English grammar conventions are hell. I will try to make it not too confusing with the word rabbit as an example noun. "Rabbit's" = single rabbit, possessive. "The rabbit's tail." "Rabbits" = plural, multiple rabbits, not possessive. "The rabbits gathered around the High Rock." Rabbits' = plural possessive. "The rabbits' nests were lined with fresh moss." 
...
- Text should be written in present tense, with the exception of history text and backstories being written in past tense. This is because patrols, events, and thoughts are happening to your rabbits at this present moment, while history text and backstories are a record of things that happened in the past.
...
- The idea that a rabbit is closer to a kit than an adult
>EG: "crawling" for paralysed rabbits - this implies they are weaker and also implies they are kit-like. This then takes away the paralysed rabbit's perceived ability to decide for themself, even though it's accidental. 
>
>INSTEAD: use "clambering", "climbing", "stalking" - these allow a paralysed rabbit to give themselves agency. 
>
...
- The idea that a rabbit "isn't" effected by their disability
>EG: "Wolfclaw... is proud that they can still work like the rest of the warren." This makes it seem like the rabbit is comparing themselves to the abled rabbits, and sets up standards where a disabled rabbit cannot show their disability. This eventually means you're implying it's somehow "lesser" to be disabled. It might make sense for an insecure rabbit, but be careful!
>
>INSTEAD: Allow the disability to effect them, and focus on their personal achievements rather than comparing them to other rabbits. 
>
...
- The idea that disabled rabbits can't make their own choices
>EG: Excessive thoughts that imply a rabbit asking for another's opinion. 
>
>INSTEAD: Balance out the thoughts. Treat it like any other rabbit!
>
>EXCEPTIONS: Personality basis - insecure/gloomy/charismatic rabbits are more likely to ask for other opinions on personal things. 

- The idea that disability is inherently worse than being abled
>EG: Thoughts that imply shame or inherent upset about being disabled, comparison to abled rabbits, etc.
>
>INSTEAD: Use benign embarrassment! Maybe a rabbit with lasting grief fell asleep during a ceremony, or a rabbit with partial hearing loss misheard a request and brought moss instead of a mole.
...
## Name Guidelines
!!! todo "TODO"
    rules regarding what is or isn't allowed in rabbit names, approval processes, and loner/kittypet names
...
### Pronoun tags
Pronoun tags are the code replacements for singular they/them within the text, as it sets up a framework for custom pronouns to be used instead of ClanGen relying on being gender neutral within the text. The pronouns are represented by {PRONOUN/rabbit/pronoun}, with rabbit representing any acronym that can be replaced by a rabbits name (m_c, r_c, p_l, s_c, etc) and pronoun representing the type of pronouns used (subjective, objective, possessive, etc). If the code was to get messed up in any way (IE misspelling subjective, or doing r_l), then it creates an error2 message within the thought.
...
### Verb tags
Verb tags, much similar to pronoun tags, serve to procedurally change verbs to match a rabbit's chosen pronoun. In ClanGen, they/them is used both singular and plural, thus the text uses verbs that make sense grammatically (are instead of is, were instead of was, or have instead of has), however custom pronouns will need their specific verbs. Consequently, verbs that need to be changed when pronouns are changed, are replaced with {VERB/rabbit/plural/singular}; rabbit representing any acronym that can be replaced by a rabbits name (m_c, r_c, p_l, etc) while plural/singular represent the versions of the verb.
...
The best ways to fix this is to first, double check that all the pronouns are properly tagged and rely on copy and pasting the actual filled out versions of the code ( {VERB/rabbit/plural/singular} ), or to copy and paste your code into [ClanGen Pronoun Tag Tester (cgen-tools.github.io)](https://cgen-tools.github.io/pronoun-tester/), which will show if the thought has an error in the tagging and makes sense given different pronouns
