...
Thoughts are accessed by going to the resources, dict, and the thought folder. The thought folder contains two separate folders based on whether the rabbit thinks the thought is alive or dead.

The alive folder contains all thoughts regarding being alive (whether the thinker is inside or outside the warren, like lost rabbits, kittypets, or rogues). Additionally, the dead folder contains all thoughts regarding being dead in the The Lightless, Inle, and the Unknown Residence.
...
### Tags
Each constraint within a thought has specific tags that limit the thought to occurring only when that constraint is fulfilled (IE, if the tag used is "leaf-bare," then the thought will only have a chance of occurring once the player Warren is in leaf-bare).
***

**ID:**
Separates the thoughts into their blocks. Generally, the ID includes the condition, personality, age, and status of the main_cat, as well as the condition, personality, age, and status of any other rabbit mentioned.
...
**SEASON:**
Constrains the thought to only occur once the Warren is in a specific season.
> "Newleaf", "Greenleaf", "Leaf-fall", "Leaf-bare",


**BURROW:**
Constrains the thought to only occur if a specific burrow type is chosen (IE “camp4”, which is the lake burrow in the forest). Furthermore, you can check the image file names of the camps to find the which number they are.
> "camp1", "camp2", "camp3","camp4",


**THOUGHTS:**
This is where the text that will be displayed in-game is placed, current abbreviations that work are r_c (for random_cat) and c_n (for player warren name).
...
**HAS_INJURIES:**            
Constraints the thought to only occur if m_c (the rabbit that is thinking the thought) or r_c (the rabbit that is being thought about) has a certain condition (either illness or injury).

> [Illness Tag List](reference/index.md#__tabbed_1_3)
>
> [Injury Tag List](reference/index.md#__tabbed_1_2)

You can additionally use the tag "any" to allow the thought to occur if the rabbit is experiencing any illness or injury.


**PERM_CONDITIONS:**
Constrains the thought to only occur if m_c (the rabbit that is thinking the thought) or r_c (the rabbit that is being thought about) has a certain perm condition. 

> [Permanent Conditions Tag List](reference/index.md#__tabbed_1_4)
>
> You can additionally use the tag "any" to allow the thought to occur if the rabbit is experiencing any permanent condition.

The additional constraint `born_with` allows you to constrain whether this thought appears for rabbits born with a condition (congenital) or not. Not providing the constraint is the same as saying either is acceptable.
...
* Do not include the name of the "thinker" (or the rabbit who is experiencing the thought)
...
Some examples of thoughts include:

    {
        ...
        "thoughts": [
            ...
            "Wonders what is happening in Inle",
            "Regrets not trying to join a different Warren before {PRONOUN/m_c/subject} died",
...
_Found in the dead folder > Unknown Residence folder > exiled.json_

    {
        "id": "fierce_deputy",
        "thoughts": [
            "Is sternly instructing a patrol about the importance of strength in defending the Warren",
...
In a quick 10 moon study with a "simple" thought (this one had one constraints), the thought appeared for most moons. However, when I did the same warren within the same amount of rabbits and with the same thought just having a single misspelled tag, it didn't occur once.
